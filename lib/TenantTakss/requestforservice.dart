...
class RequestForService extends StatelessWidget {
  RequestForService({
    super.key,
  ...
  Future<void> sendEmail(List<String> recipientEmails) async {
    ...
    final Email email = Email(
      body:
          ' A service request hase been sent by $FirstName $LastName. Please contact as soon as possible. ${_contactController.text.trim()}',
  ...
  Future<void> addServiceDetails(
    ...
    var newDocRef =
        FirebaseFirestore.instance.collection('Service_Request').doc(userID);
  ...
  FutureOr<String?> validatecontact(PhoneNumber? p1) {
    if (p1 == '') {
